...
export class Cvss2 extends CvssVector<MultiScoreResult> {
    ...
    public getRegisteredComponents(): Map<ComponentCategory, VectorComponent<VectorComponentValue>[]> {
        return Cvss2.REGISTERED_COMPONENTS;
    }
    ...
    private calculateExactImpactScore(): number {
        if (!this.isBaseFullyDefined()) return 0;
        let confidentialityImpact = (1 - this.getComponent(Cvss2.C).value);
        let integrityImpact = (1 - this.getComponent(Cvss2.I).value);
        let availabilityImpact = (1 - this.getComponent(Cvss2.A).value);
        return 10.41 * (1 - confidentialityImpact * integrityImpact * availabilityImpact);
    }

    private calculateExactExploitabilityScore(): number {
        if (!this.isBaseFullyDefined()) return 0;
        let accessComplexity = this.getComponent(Cvss2.AC).value;
        let authentication = this.getComponent(Cvss2.Au).value;
        let accessVector = this.getComponent(Cvss2.AV).value;
    ...
    private calculateExactEnvironmentalScore(): number {
        ...
        let collateralDamagePotential = this.getComponent(Cvss2.CDP).value;
        let targetDistribution = this.getComponent(Cvss2.TD).value;
    ...
    private calculateExactAdjustedImpactScore(): number {
        ...
        let confidentialityImpact = this.getComponent(Cvss2.C).value;
        let integrityImpact = this.getComponent(Cvss2.I).value;
        let availabilityImpact = this.getComponent(Cvss2.A).value;
        let confidentialityRequirement = this.getComponent(Cvss2.CR).value;
        let integrityRequirement = this.getComponent(Cvss2.IR).value;
        let availabilityRequirement = this.getComponent(Cvss2.AR).value;
    ...
    public isBaseFullyDefined(): boolean {
        return super.isCategoryFullyDefined(Cvss2.BASE_CATEGORY);
    }

    public isTemporalFullyDefined(): boolean {
        return super.isCategoryFullyDefined(Cvss2.TEMPORAL_CATEGORY);
    }

    public isEnvironmentalFullyDefined(): boolean {
        return super.isCategoryFullyDefined(Cvss2.ENVIRONMENTAL_CATEGORY);
    }

    public isAnyBaseDefined(): boolean {
        return super.isCategoryPartiallyDefined(Cvss2.BASE_CATEGORY);
    }

    public isAnyTemporalDefined(): boolean {
        return super.isCategoryPartiallyDefined(Cvss2.TEMPORAL_CATEGORY);
    }

    public isAnyEnvironmentalDefined(): boolean {
        return super.isCategoryPartiallyDefined(Cvss2.ENVIRONMENTAL_CATEGORY);
    }
    ...
    static {
        Cvss2.REGISTERED_COMPONENTS.set(Cvss2.BASE_CATEGORY, Cvss2.BASE_CATEGORY_VALUES);
        Cvss2.REGISTERED_COMPONENTS.set(Cvss2.TEMPORAL_CATEGORY, Cvss2.TEMPORAL_CATEGORY_VALUES);
        Cvss2.REGISTERED_COMPONENTS.set(Cvss2.ENVIRONMENTAL_CATEGORY, Cvss2.ENVIRONMENTAL_CATEGORY_VALUES);
    }
}
